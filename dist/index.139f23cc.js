 ...
 */ rTerm = function(options) {
    ...
    this.init = function() {
        ...
        $("#" + this.divid).html('<div id="term"> <span id="termcli">' + this.termPrev + '</span><span class="cursor">&#9608</span></div>');
    };
    ...
    this.callUpstart = function() {
        ...
        for(cid in this.data.upstart){
            if (cid > 0) delay += (this.data.upstart[cid - 1].length + 1) * this.chartime;
            setTimeout(function() {
                if (window.blurred && !this.upstartInterrupted) {
                    ...
                    return;
                } else if (!this.upstartInterrupted) this.enterCommand(this.data.upstart[this.upcid]);
                this.upcid++;
            }, delay);
        }
    ...
    this.keyCallback = (function(event) {
        if (event.key.length == 1) this.addCallback(event.key);
        else if (event.which == 8 || event.which == 46 || event.which == 110) this.delCallback();
        else if (event.which == 13) this.enterCallback();
        else if (event.which == 38) this.upCallback();
        else if (event.which == 40) this.downCallback();
    }).bind(this);
    ...
    this.enterCallback = (function() {
        if (this.input == "") this.emptyCallback();
        else {
            if (this.saveStrings) this.sendString(this.input);
            this.commandHistoryIterator = 0;
            ...
            if (args[0] in this.funcMap) this.funcMap[args[0]](args);
            else this.unknownCallback();
        }
    }).bind(this);
    // Show previous command from history
    this.upCallback = (function() {
        if (this.commandHistoryIterator < this.commandHistory.length - 1) this.commandHistoryIterator++;
        this.input = this.commandHistory[this.commandHistoryIterator];
        this.updateTerm();
    }).bind(this);
    // Show next command from history
    this.downCallback = (function() {
        if (this.commandHistoryIterator > 0) this.commandHistoryIterator--;
        this.input = this.commandHistory[this.commandHistoryIterator];
        this.updateTerm();
    }).bind(this);
    /*
    ...
    this.getByPath = (function(dstname) {
        var path = "";
        if (dstname.startsWith("/")) path = dstname;
        else if (dstname.startsWith("~")) path = this.fsstart + "/" + dstname.slice(1);
        else path = this.cdir + "/" + dstname;
        if (path.endsWith("/")) path = path.slice(0, -1);
        var pathArray = path.split("/").slice(1);
        var newPathArray = [];
        for(i in pathArray){
            if (pathArray[i] == "..") {
                if (newPathArray.length > 0) newPathArray.splice(-1, 1);
                else continue;
            } else if (pathArray[i] == ".") continue;
            else newPathArray.push(pathArray[i]);
        }
        path = "/" + newPathArray.join("/");
        var data = this.data.fs;
        for (folder of newPathArray)data = data[folder];
        return [
            data,
            path
        ];
     ...
     */ this.lsCallback = (function(args) {
        ...
        var list = false;
        var lsdir = this.cdir;
        for (arg of args.slice(1))if (!arg.startsWith("-")) {
            if (arg.startsWith("/")) lsdir = arg;
            else lsdir = lsdir + "/" + arg;
        } else {
            if (arg == "-a") all = true;
            else if (arg == "-l") list = true;
        }
        ...
        if (typeof dirData === "undefined") {
            ...
            for(item in dirData)if (!item.startsWith(".") || all) {
                if (typeof dirData[item] === "string") {
                    if (dirData[item].startsWith("_link:")) this.oldInput += '<a class="link" href="' + dirData[item].slice(6) + '" target="_blank">' + item + "</a><br>";
                    else this.oldInput += item + "<br>";
                } else this.oldInput += '<font color="#729FCF">' + item + "</font><br>";
                this.nStrings++;
            }
        }
        this.input = "";
     ...
     */ this.catCallback = (function(args) {
        ...
        if (data == "" || typeof data === "undefined") this.oldInput += this.termPrev + this.input + "<br>" + this.input + ": No such file or directory" + "<br>";
        else if (data.startsWith("_call:")) {
            var args = data.slice(6).split(" ");
            if (args[0] in this.funcMap) {
                this.funcMap[args[0]](args);
                return;
            } else this.oldInput += this.termPrev + this.input + "<br>" + data + "<br>";
        } else this.oldInput += this.termPrev + this.input + "<br>" + data + "<br>";
        this.input = "";
        this.nStrings += 2;
        this.updateTerm();
     ...
     */ this.cdCallback = (function(args) {
        var dstname = "";
        if (args.length < 2 || args[1] == " ") dstname = "~";
        else dstname = args[1];
        ...
        if (typeof data === "undefined") {
            ...
            if (pathPref == this.fsstart) pathPref = "~";
     ...
     */ this.helpCallback = (function() {
        this.oldInput += this.termPrev + this.input + "<br>GNU bash, version 4.3.48(1)-release (x86_64-pc-linux-gnu)" + '<br>These shell commands are defined internally.  Type "help" to see this list.<br><br>' + "<br>cd [-L|[-P [-e]] [-@]] [dir]" + "<br>echo [-neE] [arg ...] " + "<br>exit [n]" + "<br>help [-dms] [pattern ...]" + "<br>time [-p] pipeline" + "<br>times<br>";
        this.input = "";
        this.nStrings += 10;
        this.updateTerm();
    }).bind(this);
     ...
     */ this.sudoCallback = (function(args) {
        if (args.length == 1) {
            this.oldInput += this.termPrev + this.input + "<br>usage: sudo [options] [command]<br>";
            this.nStrings += 2;
            this.input = "";
            this.updateTerm();
        } else if (args[1] in this.funcMap) this.funcMap[args[1]](args.slice(1));
        else this.unknownCallback();
    }).bind(this);
    /*
     * Imitates df
     ...
     */ this.dfCallback = (function(args) {
        this.oldInput += this.termPrev + this.input + "<br>Filesystem  1K-blocks  Used    Available  Use%  Mounted on" + "<br>/dev/sda1   514229     514229          0  100%  /boot<br>";
        this.nStrings += 3;
        this.input = "";
        this.updateTerm();
    }).bind(this);
     ...
     */ this.aptCallback = (function(args) {
        if (args.length == 1) {
            this.oldInput += this.termPrev + this.input + "<br>apt 1.2.34 (amd64)" + "<br>Usage: apt [options] command" + "<br>This APT has Super Cow Powers.<br>";
            this.nStrings += 4;
        } else if (args[1] == "update") {
            this.oldInput += this.termPrev + this.input + "<br>Reading package lists... Done" + "<br>E: Could not open lock file /var/lib/apt/lists/lock - open (13: Permission denied)" + "<br>E: Unable to lock directory /var/lib/apt/lists/<br>";
            this.nStrings += 4;
        } else if (args[1] == "upgrade") {
            this.oldInput += this.termPrev + this.input + "<br>E: Could not open lock file /var/lib/dpkg/lock - open (13: Permission denied)" + "<br>E: Unable to lock the administration directory (/var/lib/dpkg/), are you root?<br>";
            this.nStrings += 3;
        } else if (args[1] == "install") {
            this.oldInput += this.termPrev + this.input + "<br>Reading package lists... Done" + "<br>Building dependency tree" + "<br>Reading state information... Done" + "<br>E: Unable to locate package " + args[2] + "<br>";
            this.nStrings += 5;
        } else {
            this.oldInput += this.termPrev + this.input + "<br>E: Invalid operation " + args[1] + "<br>";
            this.nStrings += 2;
        }
        this.input = "";
        this.updateTerm();
...
//# sourceMappingURL=index.139f23cc.js.map
